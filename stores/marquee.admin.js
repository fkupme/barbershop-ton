...
export const useMarqueeAdminStore = defineStore('marquee-admin', () => {
	// Состояние загрузки и ошибок
	const loading = ref(false)
	const error = ref(null)

	// Данные изображений marquee
	const images = ref([])
	...
	const fetchImages = async () => {
		...
		try {
			const response = await $fetch('/api/admin/marquee-images')
			if (response.success) {
				images.value = response.data
			}
		} catch (err) {
			error.value = err.data?.statusMessage || 'Ошибка загрузки изображений'
			console.error('Ошибка загрузки изображений marquee:', err)
		} finally {
			loading.value = false
		}
	}
	...
	const updateImage = async (id, imageData) => {
		...
		try {
			const response = await $fetch(`/api/admin/marquee-images/${id}`, {
				method: 'PUT',
				body: imageData
			})
	...
	const reorderImages = async (newOrder) => {
		...
		try {
			const response = await $fetch('/api/admin/marquee-images/reorder', {
				method: 'PUT',
				body: { order: newOrder }
			})
